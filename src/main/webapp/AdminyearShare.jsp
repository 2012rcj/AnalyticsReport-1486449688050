...
if((EStid !=null))
{
	try{
		...
		ResultSet RS = stm.executeQuery("select extract(year from retail_str_sales_detail.sale_date),round(sum(retail_str_sales_detail.total),2) from retail_str_sales_detail where retail_str_sales_detail.store_id='"+EStid+"' group by extract (year from retail_str_sales_detail.sale_date) order by extract(year from retail_str_sales_detail.sale_date) desc");
		ResultSet RS1=stm1.executeQuery("select extract(year from retail_str_sales_detail.sale_date),round(sum(retail_str_sales_detail.total),2) from retail_str_sales_detail,retail_dstr_prod where retail_dstr_prod.prod_id=retail_str_sales_detail.prod_id and retail_str_sales_detail.Store_Id='"+EStid+"' group by extract(year from retail_str_sales_detail.sale_date) order by extract(year from retail_str_sales_detail.sale_date) desc");
...
<div style="color: Indigo;"><h3><u><% out.println(V_Strnm); %></u></h3></div>
</div>
</table>
...
<tr class="even">
    <th>Year&nbsp&nbsp</th> 
  <!-- <th>Store Id&nbsp&nbsp</th> -->
...
//OutputStream file = new FileOutputStream("/home/abhinav/workspace/99ARdashDB/WebContent/PDF/Report.pdf");
OutputStream file = new FileOutputStream("PDF/Report.pdf");
PdfWriter.getInstance(d, file);
d.open();
//Image image1 = Image.getInstance("/home/abhinav/workspace/Admin/src/logo.jpg");
...
table.addCell("Market Share(In %)");
 ...
 while (RS.next() && RS1.next()) {
	...
	if(year.equals(RS1.getString(1)))
	{
	%>
		<tr class="odd">
...
<td align="RIGHT"><fmt:formatNumber type="number"  minFractionDigits="2" maxFractionDigits="2"  groupingUsed="true" value="<%=(((Float.parseFloat(RS1.getString(1)))*100)/(Float.parseFloat(RS.getString(2))))%>"/></td>
</tr>
		<%
 float marketshare = ((Float.parseFloat(RS1.getString(1)))*100)/(Float.parseFloat(RS.getString(2)));
 double s= Math.round(marketshare * 100.0)/100.0;
 String t = Double.toString(s);
// /out.println(s);
/*
 Float avgsales=((Float.parseFloat(RS.getString(2)))/(Float.parseFloat(RS.getString(2))));
...
ResultSet RS2 = stm.executeQuery("select extract(year from retail_str_sales_detail.sale_date),round(sum(retail_str_sales_detail.total),2) from retail_str_sales_detail where retail_str_sales_detail.store_id='"+EStid+"' group by extract (year from retail_str_sales_detail.sale_date) order by extract(year from retail_str_sales_detail.sale_date) desc");
		ResultSet RS3=stm1.executeQuery("select extract(year from retail_str_sales_detail.sale_date),round(sum(retail_str_sales_detail.total),2) from retail_str_sales_detail,retail_dstr_prod where retail_dstr_prod.prod_id=retail_str_sales_detail.prod_id and retail_str_sales_detail.Store_Id='"+EStid+"' group by extract(year from retail_str_sales_detail.sale_date) order by extract(year from retail_str_sales_detail.sale_date) desc");
...
while (RS2.next() && RS3.next()) {
	...
	{
 	float marketshare = ((Float.parseFloat(RS3.getString(2)))*100)/(Float.parseFloat(RS2.getString(2)));
 	 double s= Math.round(marketshare * 100.0)/100.0;
 	//out.println(s);
 	String t = Double.toString(s);
 	 Float avgsales=((Float.parseFloat(RS2.getString(2)))/(Float.parseFloat(RS2.getString(2))));
 	double avg= Math.round(avgsales * 100.0)/100.0;
 	// /out.println(s);
 	String avgsale = Double.toString(avg); 
row=sheet.createRow(c);
row.createCell((short)0).setCellValue(year);
row.createCell((short)1).setCellValue(RS2.getString(2));
row.createCell((short)2).setCellValue(RS3.getString(2));
