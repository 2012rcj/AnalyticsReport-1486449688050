            ...
            <form name="stridpage" ID="" method="POST">
                ...
                <%
                    if (request.getParameter("year") != null) {
                        try {
                            ...
                            ResultSet RS = stm.executeQuery("select to_char(to_date(extract(month from retail_str_sales_detail.sale_date),'MM'),'Month'),round(sum(retail_str_sales_detail.total),2),count(tri_id) from retail_str_sales_detail where store_id='" + EStid + "' and year(sale_date)= '2016' group by to_char(to_date(extract(month from retail_str_sales_detail.sale_date),'MM'),'Month'),extract(month from retail_str_sales_detail.sale_date)");
                            String abc = (String) request.getParameter("year");
                            ResultSet RSSUM = stmt.executeQuery("select to_char(to_date(extract(month from retail_str_sales_detail.sale_date),'MM'),'Month'),round(sum(retail_str_sales_detail.total),2),month(current_date),count(tri_id) from retail_str_sales_detail where store_id='" + EStid + "' and year(sale_date)= '2016' group by to_char(to_date(extract(month from retail_str_sales_detail.sale_date),'MM'),'Month')");
                            int count = 0;
                            int V_Count = 0;
                            double V_Sum = 0;
                            double V_Sum_bill = 0;
                            ...
                            while (RSSUM.next()) {
                                V_Sum_Array[V_Count] = RSSUM.getString(2);
                                l.add(RSSUM.getString(2));
                                storename.add(RSSUM.getString(2));
                                V_Sum_Mon_Array[V_Count] = RSSUM.getString(4);
                                V_Count++;
                            }
                            if (abc.equals("2016")) {
                ...
                <center>
                    <select onchange='if (this.value != 0) {
                        this.form.submit();
                    }' id="mytext" name="year"  style="width: 100px">
                            <%try {
                                    Class.forName("com.ibm.db2.jcc.DB2Driver");
                                    ...
                                    ResultSet RS12 = stm2.executeQuery("select extract(year from retail_str_sales_detail.sale_date) from retail_str_sales_detail where store_id = '" + EStid + " ' group by extract(year from retail_str_sales_detail.sale_date) order by extract(year from retail_str_sales_detail.sale_date) asc");

                                    while (RS12.next()) {
                                        String year = (RS12.getString(1));
                                        //out.println(year);
                                        if (RS12.getString(1).equals(request.getParameter("year"))) {
                            %>
                            <option value='<%=RS12.getString(1)%>'selected ><%=RS12.getString(1)%></option>
                            <%} else {%>
                            <option value1='<%=RS12.getString(1)%>' ><%=RS12.getString(1)%></option>
                            <%}
                                    }
                                } catch (Exception e) {
                                    out.println(e);
                    ...
                    <%
                        ...
                        float[] columnWidths = new float[]{20f, 30f, 45f};
                        ...
                        table.addCell(" Total Bills/Month");
                        ...
                        cell3.setCellValue("Total Bills/Month");
                        while (RS.next()) {
                    %>
                    <tr class="odd">
                        <TD align="LEFT"><%=RS.getString(1)%></a></td>
                    ...
                    <%
                            for (int m = 0; m < l.size(); m++) {
                                ...
                                V_Sum_bill = V_Sum_bill + Double.parseDouble(V_Sum_Mon_Array[m]);
                            }
                            for (n = 0; n < l.size(); n++) {
                                avg = ((V_Sum) / V_Sum_bill);
                ...
                <center><div style="color: darkred;"><h3><b><%
                    out.println(" Average Monthly Sales (In LKR) :-");
                    out.println((Average));
                                %></b></h3></div>
                    <center>
                        <%if (count < 1) {%>
                        ...
                        <%
                            try {
                                ...
                                ResultSet RS = stm3.executeQuery("select to_char(to_date(extract(month from retail_str_sales_detail.sale_date),'MM'),'Month'),round(sum(retail_str_sales_detail.total),2),month(current_date) from retail_str_sales_detail where store_id='" + EStid + "' and year(sale_date)= year(current_date) group by to_char(to_date(extract(month from retail_str_sales_detail.sale_date),'MM'),'Month')");
                                String abc = (String) request.getParameter("year");
                        ...
                        <center>
                            <select onchange='if (this.value != 0) {
                                    ...
                                    <option value='<%=RS12.getString(1)%>' selected><%=RS12.getString(1)%></option>
                                    <%} else {%>
                                    <option value1='<%=RS12.getString(1)%>' selected><%=RS12.getString(1)%></option>
                                    <%}
                                            }
                                        } catch (Exception e) {
                                            out.println(e);
                            ...
                            <%
                                try {
                                    ...
                                    ResultSet RS1 = stm.executeQuery("select to_char(to_date(extract(month from retail_str_sales_detail.sale_date),'MM'),'Month'),round(sum(retail_str_sales_detail.total),2),month(current_date),count(tri_id) from retail_str_sales_detail where store_id='" + EStid + "' and year(sale_date)= '2017' group by to_char(to_date(extract(month from retail_str_sales_detail.sale_date),'MM'),'Month')");
                                    ResultSet RSSUM = stmt.executeQuery("select to_char(to_date(extract(month from retail_str_sales_detail.sale_date),'MM'),'Month'),round(sum(retail_str_sales_detail.total),2),month(current_date),count(tri_id) from retail_str_sales_detail where store_id='" + EStid + "' and year(sale_date)= '2017' group by to_char(to_date(extract(month from retail_str_sales_detail.sale_date),'MM'),'Month')");
                                    int count = 0;
                                    int V_Count = 0;
                                    double V_Sum = 0;
                                    double V_Sum_bill = 0;
                            ...
                            <%
                                ...
                                table.addCell("         Month");
                                table.addCell("     Total Sales (In LKR)");
                                table.addCell(" Monthly Average Sales(In LKR)");
                                FileOutputStream fileOut = new FileOutputStream("/home/rspl-laxmeechandola/NetBeansProjects/99AR_dashdb/web/PDF/Report.xls");
                                //FileOutputStream fileOut = new FileOutputStream("PDF/Report.xls");
                                HSSFWorkbook workbook = new HSSFWorkbook();
                                HSSFSheet sheet = workbook.createSheet("New Sheet");
                                ...
                                HSSFCell cell3 = row.createCell((short) 2);
                                cell3.setCellType(HSSFCell.CELL_TYPE_STRING);
                                cell3.setCellValue("Monthly Average Sales(In LKR)");
                                while (RS.next()) {
                            %>
                            <tr class="odd">
                                <TD align="LEFT"><%=RS.getString(1)%></a></td>
                                ...
                                <div id="pageNavPosition" style="padding-top: 20px" align="center">
                                    ...
                                    <center>
                                        <select onchange='if (this.value != 0) {
                                            ...
                                            <%    //out.println("hello");
                                                while (RS12.next()) {
                                                    //out.println("hello");
                                                    select_year = (RS12.getString(1));
                                                    //out.println(abcde);
                                                    if (RS12.getString(1).equals(2017)) {
                                            %>
                                            <option value='<%=RS12.getString(1)%>' ></option>
                                            <% } else {%>
                                            <option value1='<%=RS12.getString(1)%>' selected ><%=RS12.getString(1)%></option>
                                    ...
                                    <%
                                        try {
                                            ...
                                            ResultSet RS = stm3.executeQuery("select to_char(to_date(extract(month from retail_str_sales_detail.sale_date),'MM'),'Month'),round(sum(retail_str_sales_detail.total),2),month(current_date),count(tri_id) from retail_str_sales_detail where store_id='" + EStid + "' and year(sale_date)= '2017' group by to_char(to_date(extract(month from retail_str_sales_detail.sale_date),'MM'),'Month')");
                                            ResultSet RSSUM = stmt.executeQuery("select to_char(to_date(extract(month from retail_str_sales_detail.sale_date),'MM'),'Month'),round(sum(retail_str_sales_detail.total),2),month(current_date),count(tri_id) from retail_str_sales_detail where store_id='" + EStid + "' and year(sale_date)= '2017' group by to_char(to_date(extract(month from retail_str_sales_detail.sale_date),'MM'),'Month')");
