...
<title>Hemas Sales Day Detail</title>
</head>
...
<%
  String V_saledate =(String) request.getParameter("Value1");
...
if((EStid !=null))
{
	try{
		...
		Connection con1 = DriverManager.getConnection("jdbc:db2://awh-yp-small03.services.dal.bluemix.net:50000/BLUDB:user=dash107813;password=MxX8ewhhVg7f;");
		Statement stm = con1.createStatement();
ResultSet RS = stm.executeQuery("select retail_str_sales_detail.tri_id,retail_str_sales_detail.sale_date,retail_str_sales_detail.prod_nm ,retail_str_sales_detail.qty,round((retail_str_sales_detail.total),2),retail_store_prod.th_class_1,retail_store_prod.th_class_2,retail_store_prod.th_class_3,retail_store_prod.th_class_4,retail_store_prod.mfg from retail_str_sales_detail, retail_dstr_prod, retail_store_prod where  retail_dstr_prod.prod_id=retail_str_sales_detail.prod_id and retail_str_sales_detail.prod_id=retail_store_prod.prod_id and retail_store_prod.store_id= retail_str_sales_detail.store_id and retail_str_sales_detail.store_id='"+EStid+"' and retail_str_sales_detail.sale_date= "+V_saledate+"  order by retail_str_sales_detail.tri_id asc");
%>
<table align="right">
		<tr>
		<td>
...
float[] columnWidths = new float[] {20f, 20f, 20f,20f,20f,20f,20f,20f,20f,20f};
...
cell = new PdfPCell(new Phrase((RS.getString(9))));  
         table.addCell(cell);          
cell = new PdfPCell(new Phrase((RS.getString(10))));  
         table.addCell(cell);        
}
d.add(table);
try{
		...
		Statement stmt = con2.createStatement();
                ResultSet RS1 = stm.executeQuery("select retail_str_sales_detail.tri_id,retail_str_sales_detail.sale_date,retail_str_sales_detail.prod_nm ,retail_str_sales_detail.qty,round((retail_str_sales_detail.total),2),retail_store_prod.th_class_1,retail_store_prod.th_class_2,retail_store_prod.th_class_3,retail_store_prod.th_class_4,retail_store_prod.mfg from retail_str_sales_detail, retail_dstr_prod, retail_store_prod where  retail_dstr_prod.prod_id=retail_str_sales_detail.prod_id and retail_str_sales_detail.prod_id=retail_store_prod.prod_id and retail_store_prod.store_id= retail_str_sales_detail.store_id and retail_str_sales_detail.store_id='"+EStid+"' and retail_str_sales_detail.sale_date= "+V_saledate+"  order by retail_str_sales_detail.tri_id asc ");
...
int i=0;
   ...
   int b =1;
   {
	   while(RS1.next()){
		   ...
		   row.createCell((short)9).setCellValue(RS1.getString(10));
		   b++;
     // out.println("hi");
    }
