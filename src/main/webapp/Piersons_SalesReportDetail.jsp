...
<%
String EStid=(String)request.getSession().getAttribute("abc"); //Contains StoreId from previous page 
String V_saledate =(String) request.getParameter("Value1"); // Contains sales date which is unique
...
ResultSet RS = stm.executeQuery("select distinct(retail_str_sales_detail.prod_nm),retail_str_sales_detail.Tri_id,retail_str_sales_detail.qty,round((retail_str_sales_detail.mrp),2),round((retail_str_sales_detail.total),2) ,retail_store_prod.th_class_1,retail_store_prod.th_class_2,retail_store_prod.th_class_3,retail_store_prod.th_class_4,retail_store_prod.mfg from retail_str_sales_detail join retail_store_prod on retail_str_sales_detail.prod_id=retail_store_prod.prod_id where retail_str_sales_detail.store_id= "+EStid+"  and retail_str_sales_detail.sale_date= "+V_saledate+" ");
%>
<table align="right">
		<tr>
		<td>
...
while (RS.next()) {%>
<tr class="odd">
<%--<TD align="LEFT"><a href="Hemassales2.jsp?Value1='<%= (RS.getString(1)) %>'&Value2=<%=( RS.getString(2))%>" style="color:#00FFF" style="text-decoration: none" ><%=(RS.getString(1))%></a></td>--%>
