...
<li><a href="Admin_hemassalesdaily.jsp">Daily Product</a></li>
		...
		<div style="text-align: center">
			...
			<div style="color: Indigo;">
				<h3>
					<u><b>Choose date for Top 15 Products by Value</u></b>
				</h3>
			</div>
		</div>
		</table>
		...
		<center>
			<tr>
				<td>Start Date: <input type="date" id="datepicker"
					value=<%=startdate%> placeholder="select any date"
					required="required" name="dateofexpnd" onchange="cal()" /></td>
			</tr>
			<tr>
				<td>End Date : <input type="date" id="datepicker1"
					value=<%=enddate%> placeholder="select any date"
					required="required" name="dateofexpnd1" onchange="cal()" /></td>
			</tr>
			<tr>
		...
		<center>
			...
			<tr>
				<td>Start Date: <input type="date" id="datepicker" value=""
					placeholder="select any date" required="required"
					name="dateofexpnd" onchange="cal()" /></td>
			</tr>
			<tr>
				<td>End Date : <input type="date" id="datepicker1" value=""
					placeholder="select any date" required="required"
					name="dateofexpnd1" onchange="cal()" /></td>
			</tr>
			<tr>
				<td><input type="submit" value="Submit" name="sub" id="abc"></td>
			</tr>
		...
		<%
			...
			if ((request.getParameter("sub") != null) || (request.getParameter("mail") != null)
					|| (request.getParameter("mailXls") != null)) {
				...
				try {
					...
					ResultSet RS = stm.executeQuery(
							"Select prod_nm,round(sum(total),2) from retail_str_sales_detail where sale_date between '"
									+ startdate + "' and '" + enddate + "' and store_id='" + EStid
									+ "' group by prod_nm,extract(year from sale_date),extract(month from sale_date),(DAY(LAST_DAY (sale_date))) order by sum(total) desc fetch first 15 rows only");
		%>
		<table style="white-space: nowrap;" border="1" align="center"
			id="tablepaging">
			<tr class="even">
				<th>Product Name&nbsp&nbsp</th>
				<th>Total Value &nbsp&nbsp(In LKR)</th>
			...
			<%
				Document d = new Document(PageSize.A4);
						...
						table.addCell("     Total Value (In LKR)");
						int count=0;
...
<br>
		...
		<%
			try {
						...
						cell2.setCellValue("Total Value   (In LKR)");
						int b = 1;
						{
							while (RS1.next()) {
