...
if((EStid !=null))
{
	try{
		...
		ResultSet RS = stm.executeQuery("select extract(year from retail_str_sales_detail.sale_date),to_char(to_date(extract(month from retail_str_sales_detail.sale_date),'MM'),'Month'),round(sum(retail_str_sales_detail.total),2),count(distinct (retail_str_sales_detail.sale_date)) from retail_str_sales_detail where retail_str_sales_detail.store_id='"+EStid+"' group by extract(year from retail_str_sales_detail.sale_date),extract(month from retail_str_sales_detail.sale_date),( DAY(LAST_DAY (retail_str_sales_detail.sale_date))) order by 1,2 desc");
//ResultSet RS1=stm1.executeQuery("select extract(year from retail_str_sales_detail.sale_date),to_char(to_date(extract(month from retail_str_sales_detail.sale_date),'MM'),'Month'),round(sum(retail_str_sales_detail.total),2),count(distinct(retail_str_sales_detail.sale_date)) from retail_dstr_prod,retail_str_sales_detail where retail_dstr_prod.prod_id=retail_str_sales_detail.prod_id and retail_str_sales_detail.store_id='"+EStid+"' group by extract(year from retail_str_sales_detail.sale_date),extract(month from retail_str_sales_detail.sale_date),( DAY(LAST_DAY (retail_str_sales_detail.sale_date))) order by 1,2 desc");
...
<tr class="even">
   ...
   <th>Per Day Average Sales(In LKR)</th>
    </tr>
     <%
...
table.addCell(" Per Day Average Sales(In LKR)");
int count=0;
while (RS.next()) {
	%>
		<tr class="odd">
<TD align="LEFT"><%=RS.getString(2)%></a></td>
...
<td align="RIGHT"><fmt:formatNumber type="number"  minFractionDigits="2" maxFractionDigits="2"  groupingUsed="true" value="<%=((Float.parseFloat(RS.getString(3)))/(Float.parseFloat(RS.getString(4))))%>"/></td>
</tr>
		<%
...
cell = new PdfPCell(new Phrase(avgsale));
 cell.setHorizontalAlignment(Element.ALIGN_RIGHT);
         table.addCell(cell);
         ++count; 
}
...
try{
            ...
            float sum = 0 ;
            float sum1 = 0 ;
ResultSet RS1 = stm.executeQuery("select extract(year from retail_str_sales_detail.sale_date),to_char(to_date(extract(month from retail_str_sales_detail.sale_date),'MM'),'Month'),round(sum(retail_str_sales_detail.total),2),count(distinct (retail_str_sales_detail.sale_date)) from retail_str_sales_detail where retail_str_sales_detail.store_id='"+EStid+"' group by extract(year from retail_str_sales_detail.sale_date),extract(month from retail_str_sales_detail.sale_date),( DAY(LAST_DAY (retail_str_sales_detail.sale_date))) order by 1,2 desc");

while (RS1.next()) 
 {
String month= RS1.getString(2);
String count1= RS1.getString(4);
Float avgsales=((Float.parseFloat(RS1.getString(3)))/(Float.parseFloat(RS1.getString(4))));
double avg= Math.round(avgsales * 100.0)/100.0;
// /out.println(s);
String avgsale = Double.toString(avg);
a[i] = RS1.getString(3);
...
String avgsum=String.valueOf(sum1);
...
p1.add(new Chunk ("Per Day Average Sales(In LKR) :-      "));
p1.add(new Chunk(   avgsum));
...
int i=0;
	...
	FileOutputStream fileOut = new FileOutputStream("PDF/Report.xls");
       ResultSet RS2 = stm.executeQuery("select extract(year from retail_str_sales_detail.sale_date),to_char(to_date(extract(month from retail_str_sales_detail.sale_date),'MM'),'Month'),round(sum(retail_str_sales_detail.total),2),count(distinct (retail_str_sales_detail.sale_date)) from retail_str_sales_detail where retail_str_sales_detail.store_id='"+EStid+"' group by extract(year from retail_str_sales_detail.sale_date),extract(month from retail_str_sales_detail.sale_date),( DAY(LAST_DAY (retail_str_sales_detail.sale_date))) order by 1,2 desc");
    ...
    cell3.setCellValue("Per Day Average Sales(In LKR)");
    
int b =1;
  while (RS2.next()) 
 {

Float avgsales=((Float.parseFloat(RS2.getString(3)))/(Float.parseFloat(RS2.getString(4))));
double avg= Math.round(avgsales * 100.0)/100.0;
// /out.println(s);
String avgsale = Double.toString(avg);
		   row=sheet.createRow(b);
		   row.createCell((short)0).setCellValue(RS2.getString(2));
		   row.createCell((short)1).setCellValue(RS2.getString(3));
		   row.createCell((short)2).setCellValue(avgsale);	  
		   b++;
     // out.println("hi");
