				...
				<ul class="main-navigation">
					<li><a href="#">Manufacturers Reports</a>
						<ul>
							...
							<li><a href="Admin_salesreportdaily.jsp">Daily Sales
									Report </a></li>
							<li><a href="Admin_datewisebill.jsp">Sales Bill Report </a>
							</li>
							<li><a href="#">Hemas Sales Report</a>
								<ul id="report">
									<li><a href="Admin_hemassalesdaily.jsp">Day Wise
											Product</a></li>
								</ul></li>
			...
			if ((EStid != null)) {
				try {
					...
					ResultSet r2 = stm.executeQuery(
							"select (extract(year from retail_str_sales_detail.sale_date)),round(sum(retail_str_sales_detail.total),2),count(distinct(extract(month from retail_str_sales_detail.sale_date))) from retail_str_sales_detail where retail_str_sales_detail.store_id='"
									+ EStid + "' group by extract(year from retail_str_sales_detail.sale_date)");
		%>
		<script type="text/javascript">
window.onload = function a() {
	var chart1 = new CanvasJS.Chart("chartContainer",
	{
		...
		{        
			...
			indexLabel: "Year:-{m}         AverageSales:-{y3} ",
                        indexLabelPlacement: "outside", 
                        radius: "80%",
			startAngle:80,      
			showInLegend: true,
			toolTipContent: "Total sale(In LKR)  :   {y} <br> </br>Months :  {y1} <br> </br> Average Sale (In LKR)  :  {y3}  ",
 ...
 { y1: <%=(r2.getString(3))%>,y3:<%=avg%>,y: <%=(r2.getString(2))%>, m : "<%=(r2.getString(1))%>  ",indexLabelFontColor: "orange", indexLabelOrientation: "vertical" },
    	<%}%>         
		] //closing datapoints
                }] //closing data.
	});
chart1.render();
		...
		try {
			...
			ResultSet rs = stm.executeQuery(
					"select (extract(year from retail_str_sales_detail.sale_date)),round(sum(retail_str_sales_detail.total),2),round(count(distinct (retail_str_sales_detail.sale_date))) from retail_str_sales_detail where retail_str_sales_detail.store_id='"
							+ s + "' group by extract(year from retail_str_sales_detail.sale_date)");
			//ArrayList<Integer> mylist1 = new ArrayList<Integer>();
			// Float avgsales=((Float.parseFloat(rs.getString(2)))/(Float.parseFloat(rs.getString(3))));
			//double avg= Math.round(avgsales * 100.0)/100.0;
			// /out.println(s);
			//String avgsale = Double.toString(avg);
			...
			while (rs.next()) {
				...
				// dataSet.setValue("Sale Days/Year", Integer.parseInt(rs.getString(3)));
				dataSet.setValue("Average Sales(In LKR)",
						(Float.parseFloat(rs.getString(2)) / Integer.parseInt(rs.getString(3))));
			}
		} catch (Exception e) {
			System.out.println(e);
		}
		JFreeChart chart = ChartFactory.createPieChart("Year-2016 (Average Sales)", dataSet, true, true, false);
		return chart;
	}%>
