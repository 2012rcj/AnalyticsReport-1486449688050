...
{   
	...
	Connection conn = DriverManager.getConnection("jdbc:db2://awh-yp-small03.services.dal.bluemix.net:50000/BLUDB:user=dash107813;password=MxX8ewhhVg7f;");
        ...
        ResultSet r2= stm.executeQuery("select (select round(sum(retail_str_sales_detail.total),2) from retail_str_sales_detail,retail_dstr_prod where retail_str_sales_detail.store_id='"+EStid+"'and retail_dstr_prod.prod_id=retail_str_sales_detail.prod_id and month(sale_date)=extract(month from  current date)),sum(total) from retail_str_sales_detail where month(sale_date)=extract(month from  current date) and store_id='"+EStid+"'");
...
window.onload = function a() {
	var chart1 = new CanvasJS.Chart("chartContainer",
	{
		...
		{        
			...
			indexLabel: "HemasSales:-{y1}           Share(In %)-{y}",
                         indexLabelPlacement: "outside", 
                           radius: "75%",
			startAngle:70,      
			showInLegend: true,
			//toolTipContent: "Total sale: {m} <br> </br>Hemas Sales :{x} <br> </br> Share:{x3}  ",
			legendText: "{indexLabel}",
			dataPoints: [
				<%
while (r2.next())
{ 
   Float avgsales=(((Float.parseFloat(r2.getString(1)))*100)/(Float.parseFloat(r2.getString(2))));
...
{
   y1: <%=(r2.getString(1)) %> ,y:<%=(r2.getString(2)) %> , indexLabel: "<%=(r2.getString(1)) %> " , indexLabelFontColor: "orange", indexLabelOrientation: "vertical" },
 	<% }
 %>         
		] //closing datapoints
                }] //closing data.
   ...
   try
       {
           Class.forName("com.ibm.db2.jcc.DB2Driver");
        	Connection conn = DriverManager.getConnection("jdbc:db2://awh-yp-small03.services.dal.bluemix.net:50000/BLUDB:user=dash107813;password=MxX8ewhhVg7f;");
		Statement stm =   conn.createStatement();
                ResultSet rs= stm.executeQuery("select (select round(sum(retail_str_sales_detail.total),2) from retail_str_sales_detail,retail_dstr_prod where retail_str_sales_detail.store_id='"+storeid+"'and retail_dstr_prod.prod_id=retail_str_sales_detail.prod_id and month(sale_date)=extract(month from  current date)),sum(total) from retail_str_sales_detail where month(sale_date)=extract(month from  current date) and store_id='"+storeid+"'");
while (rs.next())

                     { 
                dataSet.setValue("Hemas Sales(In LKR)", Float.parseFloat(rs.getString(1)));
                dataSet.setValue("Total Sales(In LKR)", Float.parseFloat(rs.getString(2)));
                dataSet.setValue("Market Share(In %)", ((Float.parseFloat(rs.getString(1))*100)/Float.parseFloat(rs.getString(2))));
                       } 
} catch(Exception e)
{
System.out.println(e);
				...
				"Hemas Market Share(December)", dataSet, true, true, false);
