...
{   
	...
	Connection conn = DriverManager.getConnection("jdbc:db2://awh-yp-small03.services.dal.bluemix.net:50000/BLUDB:user=dash107813;password=MxX8ewhhVg7f;");
        ...
        ResultSet r2= stm.executeQuery("select to_char(to_date(extract(month from retail_str_sales_detail.sale_date),'MM'),'Month'),round(sum(retail_str_sales_detail.total),2),round(count(distinct (retail_str_sales_detail.sale_date))),(extract(year from retail_str_sales_detail.sale_date)) from retail_str_sales_detail where retail_str_sales_detail.store_id='"+EStid+"' group by extract(year from retail_str_sales_detail.sale_date),extract(month from retail_str_sales_detail.sale_date),( DAY(LAST_DAY (retail_str_sales_detail.sale_date)))");
       ...
       
	{
		...
		{        
			...
			indexLabelFontSize: 20,
                        ...
                        //indexLabel: "Average : {y3}",
                       
                       // dataPointMaxWidth: 20,
				//color: "ORANGE",				
			//showInLegend: true,
			toolTipContent: "Total sale: {y3} <br> </br>Sale Days/month :{y1} <br> </br> Average:{y}  ",
...
{
    y1: <%=(r2.getString(3)) %>,y:<%=avg%>,y3: <%=(r2.getString(2)) %> ,y2: <%=(r2.getString(4)) %>,label: "<%=(r2.getString(1)) %>  ", indexLabelFontColor: "orange", indexLabelOrientation: "vertical"},
 	<% }
 %>         
		] //closing datapoints
                }] //closing data.
...
<%! public static JFreeChart generatePieChart(String s ) {
   ...
   try
       {
           Class.forName("com.ibm.db2.jcc.DB2Driver");
        	Connection conn = DriverManager.getConnection("jdbc:db2://awh-yp-small03.services.dal.bluemix.net:50000/BLUDB:user=dash107813;password=MxX8ewhhVg7f;");
		Statement stm =   conn.createStatement();
                ResultSet rs= stm.executeQuery("select extract(month from retail_str_sales_detail.sale_date),round(sum(retail_str_sales_detail.total),2),round(count(distinct (retail_str_sales_detail.sale_date))),(extract(year from retail_str_sales_detail.sale_date)) from retail_str_sales_detail where retail_str_sales_detail.store_id='"+s+"' group by extract(year from retail_str_sales_detail.sale_date),extract(month from retail_str_sales_detail.sale_date),( DAY(LAST_DAY (retail_str_sales_detail.sale_date))) ");
while (rs.next())

                     { 
               // dataSet.setValue("Month", Integer.parseInt(rs.getString(1)));
                dataSet.setValue("Total Sales (In LKR)", Float.parseFloat(rs.getString(2)));
               // dataSet.setValue("Sale Days/Month", Integer.parseInt(rs.getString(3)));
                dataSet.setValue("Average (In LKR)", (Float.parseFloat(rs.getString(2))/Integer.parseInt(rs.getString(3))));
                       } 
} catch(Exception e)
{
...
height = 810;
		try {
//writer = PdfWriter.getInstance(document, new FileOutputStream("/home/rspl-laxmeechandola/NetBeansProjects/99AR_dashdb/web/PDF/Report.pdf"));
...
<%
 ...
 //writeChartToPDF(generatePieChart(EStid), 500, 400, "PDF/Report.pdf");
    %>
    <%
