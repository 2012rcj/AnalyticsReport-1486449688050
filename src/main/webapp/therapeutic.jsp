...
<%@ page import="org.apache.poi.hssf.usermodel.HSSFCellStyle" %>
    ...
    <title>Therapeutic Classes </title>
    </head>
...
%>
  ...
  <table align="" border="1">
      <tr class="odd">
          <td  align="left">Therapuetic class 1</td>
          <td>
         <%
  String value=request.getParameter("Id");
  //out.println("first"+value);
  ResultSet RS = stm.executeQuery("select distinct th_cls_id_1 from retail_th_cls_ass_12") ;
  %>
  <select onchange='if(this.value != 0) { this.form.submit(); }' name="Id" style="width:152px" >
          <option>Select </option>
            <% while(RS.next()){ 
            	 if(RS.getString(1).equals(request.getParameter("Id"))) { %>
...
<td  align="left">Therapeutic Class 2</td>
<td>
<%
String value1=request.getParameter("file");
ResultSet r = stm.executeQuery("select distinct th_cls_id_2 from retail_th_cls_ass_12 where th_cls_id_1 ='"+value+"' ") ;
%>
<select onchange='if(this.value != 0) { this.form.submit(); }' name="file" style="width:152px" >
            <option>Select </option>
<% while(r.next())
{ 
         if(r.getString(1).equals(request.getParameter("file"))) 
                 { %>
                 <option value='<%=r.getString(1)%>' selected><%=r.getString(1)%></option>
            <% }
else { %>
      <option value1='<%=r.getString(1)%>' ><%=r.getString(1)%></option>
             <%}
} %>
</select>
</td>
<td  align="left">Therapeutic Class 3</td>
<td>
<%
String value2=request.getParameter("file1");
ResultSet r2 = stm.executeQuery("select distinct th_cls_id_3 from retail_th_cls_ass_23 where th_cls_id_2 ='"+value1+"' ") ;
%>
<select  onchange='if(this.value != 0) { this.form.submit(); }' name="file1" style="width:152px" >


<option>Select </option>      
<% while(r2.next())
{ 
    if(r2.getString(1).equals(request.getParameter("file1"))) 
    { %>
<option value='<%=r2.getString(1)%>' selected><%=r2.getString(1)%></option>
 <% }
    else 
    { %>
<option value1='<%=r2.getString(1)%>' ><%=r2.getString(1)%></option>
    <%}
} %>
</select>
</td>
 
           <td  align="left">Therapeutic Class 4</td>
<td>
<%
String value3=request.getParameter("file2");
ResultSet r3 = stm.executeQuery("select distinct th_cls_id_4 from retail_th_cls_ass_34 where th_cls_id_3 ='"+value2+"' ") ;
%>
<select onchange='if(this.value != 0) { this.form.submit(); }' name="file2" style="width:152px" >
<option>Select </option>    
<% while(r3.next()){ 
            	 if(r3.getString(1).equals(request.getParameter("file2"))) 
                 { %>
                 <option value='<%=r3.getString(1)%>' selected><%=r3.getString(1)%></option>
            <% }
                else { %>
                    <option value1='<%=r3.getString(1)%>' ><%=r3.getString(1)%></option>
             <%} } %>
</select>
</td>
</tr>
...
//String G = request.getParameter("Id");
 ...
 <table style="white-space: nowrap;" border="1" align="center" id="tablepaging">
                <tr class="even">
                    <th>Product Name</th>
                    <th>Sale Value(In LKR)</th>
                    <th>Therapeutic Class 1 </th>
                    <th>Therapeutic Class 2 </th>
                    <th>Therapeutic Class 3 </th>
                    <th>Therapeutic Class 4 </th>
                 </tr>
...
d.open();
//Image image1 = Image.getInstance("/home/abhinav/workspace/99AR_dashdb/WebContent/PDF/logo.jpg");
...
float[] columnWidths = new float[] {40f, 35f, 40f,40f,40f,40f};
...
table.addCell("Therapeutic Class 1");
table.addCell("Therapeutic Class 2");
table.addCell("Therapeutic Class 3");
table.addCell("Therapeutic Class 4");
...
ResultSet RS1 = stm1.executeQuery("select retail_store_prod.prod_nm, retail_str_sales_detail.total from retail_store_prod , retail_str_sales_detail where retail_str_sales_detail.prod_nm=retail_store_prod.prod_nm and retail_store_prod.th_class_1='"+request.getParameter("Id")+"' and retail_store_prod.th_class_2='"+request.getParameter("file")+"' and retail_store_prod.th_class_3='"+request.getParameter("file1")+"' and retail_store_prod.th_class_4='"+request.getParameter("file2")+"' and retail_store_prod.store_id=retail_str_sales_detail.store_id and retail_str_sales_detail.store_id='"+EStid+"' ORDER BY retail_store_prod.prod_nm ") ;
  while (RS1.next()) { 
  %>
<tr class="odd">
<TD align="left"><%=(RS1.getString(1))%></td>
<TD align="right"><fmt:formatNumber type="number"  minFractionDigits="2" maxFractionDigits="2"  groupingUsed="true" value="<%=(RS1.getString(2))%>" /></td>
<TD align="left"><%=request.getParameter("Id")%></td>
<TD align="left"><%=request.getParameter("file")%></td>
<TD align="left"><%=request.getParameter("file1")%></td>
<TD align="left"><%=request.getParameter("file2")%></td>
</tr>
<%
  
cell = new PdfPCell(new Phrase(RS1.getString(1)));
table.addCell(cell);
cell = new PdfPCell(new Phrase(RS1.getString(2))); 
 cell.setHorizontalAlignment(Element.ALIGN_RIGHT);
table.addCell(cell);  
cell = new PdfPCell(new Phrase(request.getParameter("Id")));  
table.addCell(cell);
cell = new PdfPCell(new Phrase(request.getParameter("file")));  
table.addCell(cell); 
 cell = new PdfPCell(new Phrase(request.getParameter("file1")));
table.addCell(cell);
 cell = new PdfPCell(new Phrase(request.getParameter("file2")));  
table.addCell(cell); 
...
<%
      ...
      FileOutputStream fileOut = new FileOutputStream("PDF/Report.xls");
        ...
        ResultSet RS3 = stm3.executeQuery("select retail_store_prod.prod_nm, retail_str_sales_detail.total from retail_store_prod , retail_str_sales_detail where retail_str_sales_detail.prod_nm=retail_store_prod.prod_nm and retail_store_prod.th_class_1='"+request.getParameter("Id")+"' and retail_store_prod.th_class_2='"+request.getParameter("file")+"' and retail_store_prod.th_class_3='"+request.getParameter("file1")+"' and retail_store_prod.th_class_4='"+request.getParameter("file2")+"' and retail_store_prod.store_id=retail_str_sales_detail.store_id and retail_str_sales_detail.store_id='"+EStid+"' ORDER BY retail_store_prod.prod_nm");
    ...
    cell2.setCellValue("Sales Value (In LKR)");
    
    HSSFCell cell3 = row.createCell((short) 2);
    cell3.setCellType(HSSFCell.CELL_TYPE_STRING);
    cell3.setCellValue("Therapeutic Class 1");
    
    HSSFCell cell4 = row.createCell((short) 3);
    cell4.setCellType(HSSFCell.CELL_TYPE_STRING);
    cell4.setCellValue("Therapeutic Class 2");
    
    HSSFCell cell5 = row.createCell((short) 4);
    cell5.setCellType(HSSFCell.CELL_TYPE_STRING);
    cell5.setCellValue("Therapeutic Class 3");
    
    HSSFCell cell6 = row.createCell((short) 5);
    cell6.setCellType(HSSFCell.CELL_TYPE_STRING);
    cell6.setCellValue("Therapeutic Class 4");
   int b =1;
  //out.println(request.getParameter("file"));
	   while(RS3.next()){
		   row=sheet.createRow(b);
		   row.createCell((short)0).setCellValue(RS3.getString(1));
		   row.createCell((short)1).setCellValue(RS3.getString(2));
		   row.createCell((short)2).setCellValue(request.getParameter("Id"));
		   row.createCell((short)3).setCellValue(request.getParameter("file"));
		   row.createCell((short)4).setCellValue(request.getParameter("file1"));
		   row.createCell((short)5).setCellValue(request.getParameter("file2"));
		   b++;
    // out.println("hi");
    }
