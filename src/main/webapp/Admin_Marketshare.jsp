...
ResultSet RS = stm.executeQuery("select round(sum(grand_total),2) from retail_str_sales_master where store_id='"+EStid+"'");
ResultSet RS1 = stm1.executeQuery("select round(sum(retail_str_sales_detail.total),2) from retail_dstr_prod,retail_str_sales_detail where retail_str_sales_detail.store_id='"+EStid+"'and  retail_dstr_prod.prod_id=retail_str_sales_detail.prod_id ");
%>
...
<tr class="even">
   ...
   <th>Total Sales (In LKR)</th>
   <th>Hemas Sales &nbsp&nbsp(In LKR)</th>
      <th>Market Share &nbsp&nbsp(In %)</th>
     </tr>
     <%
     Document d=new Document(PageSize.A4);
     //OutputStream file = new FileOutputStream("/home/abhinav/workspace/Admin/WebContent/PDF/Report.pdf");
...
table.setTotalWidth(20f);
table.addCell("         Market Share(%)");
table.setTotalWidth(20f);
while (RS.next() && (RS1.next())) 
{%>
<tr class="odd">
<TD align="right"><fmt:formatNumber type="number" minFractionDigits="2" maxFractionDigits="2" groupingUsed="true" value="<%=(RS.getString(1))%>"/></td>
<TD align="right"><fmt:formatNumber type="number" minFractionDigits="2" maxFractionDigits="2" groupingUsed="true" value="<%=(RS1.getString(1)) %>" /></td>
<td align="right"><fmt:formatNumber type="number"  minFractionDigits="2" maxFractionDigits="2"  groupingUsed="true" value="<%=(((Float.parseFloat(RS1.getString(1)))*100)/(Float.parseFloat(RS.getString(1))))%>"/></td>
</tr>
<%
float a = ((Float.parseFloat(RS1.getString(1)))*100)/(Float.parseFloat(RS.getString(1)));
//out.println(a);
...
email.setSubject("HEMAS PRODUCT SALES");
